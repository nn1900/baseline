...
Object.assign(MySql.prototype, {
  ...
  getTableColumns(db, table) {
    const sql = [
      'select column_name, column_type, column_default, data_type, is_nullable, column_comment, extra',
  ...
  exportTableSchema(db, table) {
    return Promise.all([
      ...
      const columns = res[2];
      return [
        mysql.format('CREATE TABLE ??.?? (', [db, table]),
        [
  ...
  exportTableConstraints(db, table) {
    ...
    return this.query(sql).then(res => res.map(x => x.sql));
  },
  ...
  exportTableIndexes(db, table) {
    const sql = mysql.format([
      ...
      "FROM information_schema.STATISTICS O WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? AND UPPER(INDEX_NAME) != 'PRIMARY' ",
    ...
    return this.query(sql).then(res => res[0].sql);
  },
